...
getAllCafix():Observable<any>{
  return this.http.get(BASIC + '/api/customer/all-carfix',{headers:this.createAuthorizationHeader()});

}

getAllPaymentByUserId():Observable<any>{
  return this.http.get(BASIC + `/api/customer/${StorageService.getUserId()}`,{headers:this.createAuthorizationHeader()});
}

getCarFixById(carFixId:number):Observable<any>{
  return this.http.get(BASIC + `/api/customer/carFix/${carFixId}`,{headers:this.createAuthorizationHeader()});
}
